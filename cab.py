...
with BuildPart() as cab_red:
    cap_color = Color("red")
    # cap_color = body_color
    with BuildSketch() as cab_red_plan:
        RectangleRounded(16, 16, 1)
        split(bisect_by=Plane.YZ)

    extrude(amount=7, dir=(0, 0.15, 1))
    fillet(cab_red.edges().group_by(Axis.Z)[-1].group_by(Axis.X)[1:], 1)
    ...
    cab_red.part.move(Location((0, -7, 8.5)))
    cab_red.part.color = cap_color

with BuildPart() as cab_green:
    cab_color = Color("green")
    with BuildSketch() as cab_green_plan:
        RectangleRounded(16, 16, 1)
        split(bisect_by=Plane.YZ)

    extrude(amount=7, dir=(0, 0.15, 1))
    fillet(cab_green.edges().group_by(Axis.Z)[-1].group_by(Axis.X)[1:], 1)
    ...
    cab_green.part.move(Location((0, -2, 8.5)))
    cab_green.part.color = cab_color
